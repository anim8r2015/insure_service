...
final class JarURLConnection extends java.net.JarURLConnection
{
    ...
    static JarURLConnection get(final URL url, JarFile jarFile) throws IOException {
        final String spec = extractFullSpec(url, jarFile.getPathFromRoot());
        if (spec == null) {
        ...
        for (index = 0; (separator = spec.indexOf("!/", index)) > 0; index = separator + "!/".length()) {
            final JarEntryName entryName = JarEntryName.get(spec.substring(index, separator));
            final JarEntry jarEntry = jarFile.getJarEntry(entryName.toString());
            if (jarEntry == null) {
